...
register_option truth_table.high_contrast : Bool := {
  defValue := Bool.true
  descr := "Uses high-contrast colors in truth table."
}
...
def htmlOfTable (t : List (List (String × Bool))) (high_contrast : Bool := false) : String :=
  -- Hacky workaround
  if h1 : t = [] then "" else
  "<table cellpadding=\"5\""
  ...
  ++ List.foldl (λ acc r =>
      acc ++ "<tr>"
          ...
          let bg := match (high_contrast, txt) with
            | (true, "true") => "lightgreen"
            | (true, "false") => "crimson"
            | (false, "true") => "darkgreen"
            | (false, "false") => "darkred"
            | _ => "white"
          let txt_color := match (high_contrast, txt) with
            | (true, "true") => "black"
            | (true, "false") => "white"
            | (false, "true") => "white"
            | (false, "false") => "white"
            | _ => "white"
          acc ++ "<td style=\"border:1px solid gray; color: " ++ txt_color ++ "; background-color:"
              ++ bg ++ "\">" ++ toString p.2 ++ "</td>") "" ++ "</tr>") "" t
  ++ "</tbody></table>"

def mkTableWidget (t : List (List (String × Bool))) (high_contrast : Bool := false) :
...
@[command_elab «truthTableCommand»] private unsafe def elabTableWidget :
  ...
  | stx@`(#truth_table $prop) => do
    ...
    let high_contrast ←
      if (← getOptions).getBool `truth_table.high_contrast true then `(true) else `(false)

    elabDeclaration (←
      `(@[widget] def $ident :=
        mkTableWidget
          (truthTable $(← bExprOfPropTerm prop))
          $(high_contrast))
